...
from src.ariel.body_phenotypes.lynx_robotics.tools.math_utils import Vector3, Quaternion, ensure_list
...
class Base(Module):
    ...
    def build(self, mjcf, entry_point, attachment_point_pos, attachment_point_quat) -> list:
        ...
        root_body.add("geom", type="cylinder", size=[self.base_radius1, self.base_length1 / 2], pos=[0, 0, self.base_length1 / 2])
        # <geom type="cylinder" size="0.075 0.0072" pos="0 0 0.0922"material="black" class="visual"/>
        root_body.add("geom", type="cylinder", size=[self.base_radius2, self.base_length2 / 2], pos=[0, 0, self.base_length1 + self.base_length2 / 2])
